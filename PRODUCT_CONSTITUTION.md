...
**Home** is where a child meets Halo. Not a door and not a dashboard: the room the companion lives in. The whole product is presented in the dark now, so Home is no longer the exception it once was — but it is still the most lit room in it. The order of it is greeting, Halo, the chapter to carry on with, then the ways further in. Home arrives whole: it is one screenful by construction, because a child coming back for the story they left should see it without scrolling for it. Halo is the part that yields — it takes everything the screen has left once the words and the doors have taken theirs, which on a tall phone is more of the screen than anything else on it and on a small one is less. Nothing else gives way first, and nothing is ever pushed past the fold. Nothing there counts anything; "continue learning" names a chapter, which is a fact about the content and not a measurement of the child. The three ways further in are three plain doors side by side, each one mark and one word — what keeps that from being a dashboard is not their shape but what is on them: no counts, no progress rings, no "3 left". They say only where they go. **Chapters** is the shelf. **Games** is every chapter's activities in one place, and will shuffle between them as chapters gain more than one — still no levels, locks or scores. **Verses** becomes the child's growing collection of verses from chapters they have read.
