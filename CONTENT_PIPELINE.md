...
> reader — and where the two disagree, it wins.
>
> **Revised by `CONTENT_WORKFLOW.md`** on where curriculum comes from. Google
> Drive holds the curriculum as photographed or scanned book pages, and the
> sheet's Curriculum column points at that folder rather than asking a teacher
> to retype the lesson. §M below recorded the opposite recommendation and now
> records why it was reversed. Everything else here stands — the sheet's shape,
> the brief snapshot, the generation step and the validation are unchanged.
...
| 4 | **Curriculum** | teacher | Where the lesson comes from. As built, this is the link to the chapter's Drive `Curriculum` folder — the photographed Samajham pages. Free text may be added but is not required. See `CONTENT_WORKFLOW.md`; the column was named "What this lesson is about" when this was written. |
...
> **Reversed. See `CONTENT_WORKFLOW.md`, which is now normative for this
> section.** Drive is the curriculum source, not an optional extra. The
> recommendation below is kept because the reasoning is worth reading and
> because the premise it turned on is exactly what changed.

**The recommendation was to skip Drive for V1**, on this reasoning:

> Drive is worth its complexity when contributors supply binaries. In this
> design they do not — every asset is generated by you, in the same run that
> generates the content that references it, and lands directly in the repo.
> Adding Drive would mean a sync step, an ID mapping, and a second place for an
> asset to be stale.

**The premise was wrong, and it was wrong about the input rather than the
output.** Contributors do supply binaries: the official curriculum already
exists as pages of the Sunday School Samajham books, and the fastest way for a
teacher to contribute a lesson is to photograph the pages they are already
holding. The alternative this document assumed — a teacher typing the lesson
into the "What this lesson is about" cell — is asking for work that has already
been done, in a format that loses the original.

So Drive is the **source**, not an asset store:

```
Drive        the curriculum, as pages          (contributors upload)
Sheets       what we are doing with it         (contributors and you)
Repository   what the child experiences        (you, reviewed as a diff)
```

The concern about "a second place for an asset to be stale" does not apply,
because nothing in Drive is ever an app asset. Curriculum pages are read by the
generation workflow and stay in Drive; production artwork is generated into
`public/art/` and lives only there. The two trees never hold the same file.

What survives from this section unchanged: the naming convention below is what
a Drive folder syncs into, and no runtime dependency on Drive is created. The
app still never contacts Google.
